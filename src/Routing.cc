...
void Routing::handleMessage(cMessage *msg)
{
    ...
    if (msg->arrivedOn("localIn")) {
        ...
        PacketInfo *info = check_and_cast<PacketInfo *> (pkt->removeControlInfo());
        pk = new Packet(pkt->getName());
        pk->setDestAddr(info->getDestAddr());
        if (0 <= info->getPort() && info->getPort() < outputQueues.size())
            outGateIndex = info->getPort();
        pk->setHopCount(0);
    ...
    if (destAddr == myAddress) {
        ...
        cPacket *pkt = pk->decapsulate();
        PacketInfo *info = new PacketInfo();
        info->setSrcAddr(pk->getSrcAddr());
        info->setHopCount(pk->getHopCount());
